...
import { isNativeSymbolOrAddress, isNativeWrappedSymbolOrAddress } from 'ogre-token-metadata';
...
import { ERC20BridgeSource, RfqRequestOpts, SupportedProvider, SwapQuoterError } from '../asset-swapper';
import {
...
export class SwapHandlers {
    ...
    constructor(swapService: ISwapService, provider: SupportedProvider) {
        ...
        try {
            ...
            if (CHAIN_ID === ChainId.Arbitrum) {
                const gasUtils = GasPriceUtils.getInstance(
                    ...
                    ZERO_EX_GAS_API_URL,
                    new Web3Wrapper(this._provider),
                );
